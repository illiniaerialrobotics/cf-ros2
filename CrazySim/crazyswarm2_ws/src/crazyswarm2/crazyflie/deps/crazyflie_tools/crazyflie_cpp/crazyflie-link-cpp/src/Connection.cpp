    ...
    : impl_(std::make_shared<ConnectionImpl>())
{
  ...
  // "udp://hostname:port" -> connect over UDP

  impl_->uri_ = uri;

  // Check for UDP URI first
  const std::regex udp_regex("udp:\\/\\/([^:]+):(\\d+)");
  std::smatch udp_match;
  if (std::regex_match(uri, udp_match, udp_regex)) {
    impl_->isRadio_ = false;
    impl_->isUDP_ = true;
    impl_->hostname_ = udp_match[1].str();
    impl_->port_ = static_cast<uint16_t>(std::stoi(udp_match[2].str()));
    impl_->devid_ = -1;

    udpThread_ = std::unique_ptr<CrazyflieUDPThread>(
        new CrazyflieUDPThread(impl_->hostname_, impl_->port_));
    udpThread_->addConnection(impl_);
    return;
  }
...
void Connection::close()
{
  if (impl_->isUDP_) {
    if (udpThread_) {
      udpThread_->removeConnection(impl_);
      udpThread_.reset();
    }
